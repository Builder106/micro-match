...
The locale prefix controls both UI messages and the task-content translation target. For example, `/fr/tasks` renders the French interface and requests French task fields; task detail pages automatically show that locale when a translation is available. The task’s source language remains metadata, and the detail page provides an ephemeral original/translation toggle without changing the URL or locale cookie. The original task is the fail-open fallback when translation is unavailable. The locale cookie persists the choice, while `Accept-Language` is used only for the first redirect from an unlocalized request.
