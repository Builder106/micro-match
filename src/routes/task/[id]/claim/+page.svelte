<script lang="ts">
  ...
  async function submitClaim() {
    ...
    try {
      ...
      window.location.href = resolve('/dashboard?celebrate=1');
    } catch (e) {
      console.error(e);
...
<div class="cl-page">
  ...
  <section class="cl-hero brand-card">
    <div class="cl-hero-blob" aria-hidden="true"></div>
  ...
  <!-- ───── Task summary card ───── -->
  <section class="cl-task brand-card" aria-labelledby="claim-task-title">
    ...
    <div class="cl-task-meta">
      ...
      <h2 id="claim-task-title">{data.task.title}</h2>
...
<style>
  ...
  @media (max-width: 768px) { .cl-hero-blob { display: none; } }
  :global(html[dir='rtl']) .cl-hero-blob { display: none; }
  .cl-hero-text { position: relative; z-index: 1; max-width: 540px; padding: 8px 12px; border-radius: 12px; background: var(--color-surface); }
  ...
  .cl-hero-text p { color: var(--color-text-secondary); font-size: 15px; line-height: 1.6; margin: 0; }
  ...
  .cl-task-meta small { font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.08em; color: var(--color-text-secondary); }
  .cl-task-meta h2 { font-size: 17px; font-weight: 700; margin: 0; line-height: 1.3; }
  .cl-task-meta p { font-size: 13px; color: var(--color-text-secondary); margin: 0 0 4px; line-height: 1.5; }
  ...
  .cl-form-head p { color: var(--color-text-secondary); font-size: 13px; margin: 0; line-height: 1.5; }
  ...
  .cl-label em { font-style: normal; font-size: 11px; font-weight: 600; padding: 2px 6px; background: var(--color-error-container); color: var(--color-error); border-radius: 9999px; text-transform: uppercase; letter-spacing: 0.04em; }
  .cl-em-optional { background: var(--color-surface-variant) !important; color: var(--color-text-secondary) !important; }
  ...
  .cl-hint { font-size: 12px; color: var(--color-text-secondary); }
  .cl-counter { font-size: 12px; font-weight: 600; color: var(--color-text-secondary); }
